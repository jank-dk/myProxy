...
"test" - matches any method on path test
If a path ends with a slash, if will match any url starting with path:
"GET /" - matches all GET requests (put this last if you have other rules you want matched as well)

Usage of myProxy:
  -hostname string
        The host name to serve TLS for
  -pemfile string
        The file where the certificate and private key is stored for the CA (default "ca.pem")
  -port int
        The port to listen on (default 443)
  -rules string
        What file to read the proxy rules from (default "rules.json")
  -verbose
        Verbose output
